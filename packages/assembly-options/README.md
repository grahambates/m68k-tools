...
Private, shared options for how a project's source is assembled, read once so the tools agree: the processors it targets, the directory the assembler is run in (`sourceRoot`), the directories it searches for includes, whether symbol names keep their case, and whether strings read backslash escapes (`-esc`).
