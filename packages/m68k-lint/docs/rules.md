...
| [`optimization/divs-word-by-constant`](rule-examples.md#optimizationdivs-word-by-constant) | suggestion | yes | Replace signed word division by a constant with a multiply by its reciprocal when only the quotient of a small dividend is needed | Fixed-point reciprocal multiplication: the scale is chosen per divisor, with a correction for negative dividends, and the exact range checked over every dividend |
